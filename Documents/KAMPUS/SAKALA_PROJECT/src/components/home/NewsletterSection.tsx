...
export default function NewsletterSection() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setStatus({ success: false, message: 'Terjadi kesalahan. Silakan coba lagi.' });
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <section id="informasi-terbaru" className="relative bg-[#070F18] text-white py-20 lg:py-28 overflow-hidden border-b border-[#C5AA00]/20">
      ...
      <div className="max-w-4xl mx-auto px-6 text-center relative z-10">
        <ScrollReveal direction="up" delay={50}>
          ...
          <h2 className="font-serif-editorial text-3xl sm:text-4xl lg:text-5xl font-black text-white tracking-tight mb-2">
            DAPATKAN <span className="text-[#C5AA00]">INFORMASI TERBARU</span>
          </h2>

          {/* Subtitle */}
          <p className="text-xs text-[#94A3B8] max-w-md mx-auto mb-8 leading-relaxed">
            Daftarkan email Anda untuk menerima informasi rilisan apparel terbaru, dokumentasi kustom motor, dan agenda kegiatan resmi Sakala.
          </p>
        </ScrollReveal>
        ...
        <ScrollReveal direction="up" delay={120}>
          <form onSubmit={handleSubmit} className="max-w-lg mx-auto mb-4">
            <div className="flex flex-col sm:flex-row items-stretch gap-2 bg-white/5 p-1.5 border border-[#C5AA00]/30 rounded-xs backdrop-blur-xs transition-all duration-300 focus-within:border-[#C5AA00]">
              ...
              >
                <span>{loading ? 'MEMPROSES...' : 'DAPATKAN INFORMASI'}</span>
        ...
        <p className="text-[10px] text-[#64748B] max-w-md mx-auto">
          Informasi langsung dari Bandung HQ. Tanpa spam, hanya pembaruan penting.
        </p>
      </div>
    </section>
  );
